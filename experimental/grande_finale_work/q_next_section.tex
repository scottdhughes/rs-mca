...
has the form \(h(X^e)\).  For a multiplicative coset \(S\) of order \(N\),
put \(c=\gcd(e,N)\).  Then
\[
...
\]
Thus this mass lives on the image coset \(S^e\) with multiplicity \(c\),
exactly as in \cref{prop:composite-descend}.  When \(c>1\), any future Q
theorem must first charge these divisor-lattice contributions to the quotient
ledger, and then bound the remaining primitive max fibers.
\end{remark}
