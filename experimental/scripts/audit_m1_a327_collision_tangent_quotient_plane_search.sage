#!/usr/bin/env sage
"""Sage gate for the M1 a=327 collision-tangent quotient-plane packet."""
...
def audit_record():
    ...
    record["sage_gate"] = {
        ...
        "reason": (
            "proxy tangent-plane candidate pending exact lift"
            if record["exact_audit"]["triggered"]
            else "no proxy collision-tangent quotient-plane a>=327 candidate"
        ),
    }
    return record
...
def main():
    ...
    if args.json:
        ...
        print("SAGE_GATE_M1_A327_COLLISION_TANGENT_QUOTIENT_PLANE_SEARCH_OK")
