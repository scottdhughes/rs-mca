#!/usr/bin/env sage
"""Sage gate for the M1 a=327 tangent-skeleton Hall repair packet."""
...
def audit_record():
    ...
    record["sage_gate"] = {
        ...
        "reason": (
            "tangent-skeleton Hall proxy candidate pending exact lift"
            if record["exact_audit"]["triggered"]
            else "no tangent-skeleton Hall proxy a>=327 candidate"
        ),
    }
    return record
...
def main():
    ...
    if args.json:
        ...
        print("SAGE_GATE_M1_A327_TANGENT_SKELETON_HALL_REPAIR_OK")
