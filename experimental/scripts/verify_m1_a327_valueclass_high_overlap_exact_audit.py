#!/usr/bin/env python3
"""Dependency-free verifier for the M1 a=327 value-class high-overlap exact audit."""
...
def verify() -> dict[str, Any]:
    ...
    require(record["construction_mode"] == "valueclass_high_overlap_exact_audit", "wrong mode")
    ...
    require(record["source"]["source_retained_proxy_count"] == source["rank_gate"]["retained_proxy_count"], "source retained count mismatch")

    results = record["results"]
    require(record["exact_audited_count"] == len(results) == 6, "wrong exact audit count")
    ...
    for row in results:
        ...
        require(row["min_support_size"] == scanner.TARGET_AGREEMENT, "min support mismatch")
        ...
        require(row["pairs_at_or_above_250"] == summary["pairs_at_or_above_250"], "250 pair count mismatch")
        require(row["membership_histogram"] == summary["membership_histogram"], "histogram mismatch")
        require(set(row["membership_size_support"]).issubset({3, 4, 5, 6}), "unexpected membership size")
        require(sum(row["membership_histogram"].values()) == scanner.N, "histogram does not cover H")
...
def main() -> None:
    ...
    if args.json:
        ...
        print(
            "PASS: M1 a=327 value-class high-overlap exact audit "
