#!/usr/bin/env sage
"""Exact GF(17^32) audit wrapper for the joint target/codeword solver."""
...
DATA_PATH = Path("experimental/data/m1_a327_joint_target_codeword_solver_exact_audit.json")
...
def audit_record():
    ...
    # The proxy scanner triggers exact Sage extraction only after a proxy
    # received-word assignment reaches a>=327.  The current bounded layer has
    # no such trigger; recording that fact is the exact audit boundary.
    if triggers:
        raise RuntimeError(
            "exact target-system extraction is intentionally gated; "
            "review trigger systems before promoting them to Sage extraction"
        )
    ...
    return jsonable(
        {
            ...
            "construction_mode": "joint_target_codeword_solver_exact_audit",
            "source": {
...
def main():
    ...
    if not args.json:
        print("SAGE_AUDIT_M1_A327_JOINT_TARGET_CODEWORD_SOLVER_OK")
