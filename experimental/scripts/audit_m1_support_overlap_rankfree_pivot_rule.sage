#!/usr/bin/env sage
"""Sage audit for the M1 support-overlap rank-free pivot-rule packet."""
...
def row_sort_key(meta):
    i, j = [int(part) for part in meta["pair"].split(",")]
    return i, j, int(meta["position"])


def schedule_rules(row_meta, ncols):
    pair_sizes = {}
    fiber_sizes = {}
    for meta in row_meta:
        pair = meta["pair"]
        fiber = int(meta["position"]) % 16
        pair_sizes[pair] = pair_sizes.get(pair, 0) + 1
        fiber_sizes[fiber] = fiber_sizes.get(fiber, 0) + 1

    indexed = list(enumerate(row_meta))
    rules = {
        "matrix_order_first_n": sorted(indexed, key=lambda item: item[0]),
        "reverse_matrix_order_first_n": sorted(indexed, key=lambda item: -item[0]),
        "pair_label_coordinate_order": sorted(
            indexed, key=lambda item: row_sort_key(item[1])
        ),
        "coordinate_pair_order": sorted(
            indexed,
            key=lambda item: (
                int(item[1]["position"]),
                tuple(int(part) for part in item[1]["pair"].split(",")),
            ),
        ),
        "pair_pressure_desc": sorted(
            indexed,
            key=lambda item: (
        ...
        "fiber_pressure_desc": sorted(
            indexed,
            key=lambda item: (
                -fiber_sizes[int(item[1]["position"]) % 16],
                int(item[1]["position"]) % 16,
                tuple(int(part) for part in item[1]["pair"].split(",")),
                int(item[1]["position"]),
            ),
        ),
        "fiber_coordinate_order": sorted(
            indexed,
            key=lambda item: (
                int(item[1]["position"]) % 16,
                int(item[1]["position"]),
    ...
    return {rule: [idx for idx, _meta in rows[:ncols]] for rule, rows in rules.items()}


def rule_attempts(base, M, row_meta):
    ncols = M.ncols()
    cols = list(range(ncols))
    out = []
    for rule, rows in sorted(schedule_rules(row_meta, ncols).items()):
        ...
        out.append(
            {
                "rule": rule,
                "rule_class": "DETERMINISTIC_COMBINATORIAL_RULE",
                ...
                "status": "SUCCESS" if minor_rank == ncols else "FAILED_SINGULAR_MINOR",
            }
        )
    return out
        ...
        rows.append(
            {
                ...
                "rankfree_rule_attempts": rule_attempts(base, M, row_meta),
...
def validate_record(record, computed):
    ...
    for schedule in record["matrix_schedules"]:
        ...
        assert schedule["rankfree_rules"] == computed_row["rankfree_rule_attempts"]
...
def main():
    ...
    if args.json:
        ...
        print("SAGE_AUDIT_M1_SUPPORT_OVERLAP_RANKFREE_PIVOT_RULE_OK")
