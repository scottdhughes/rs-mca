#!/usr/bin/env sage
"""Sage audit for the M1 a=327 pairwise-divisibility null-vector systems."""
...
SCANNER_PATH = Path("experimental/scripts/scan_m1_a327_pairwise_divisibility_nullvector_system.py")
...
def load_scanner():
    spec = importlib.util.spec_from_file_location("m1_a327_pairwise_divisibility_scanner", SCANNER_PATH)
...
def build_compressed_matrix(F, H, partitions):
...
def rank_gate(partitions, field_mode):
    scanner = load_scanner()
    q, F, H, label = field_context(field_mode)
    compressed = build_compressed_matrix(F, H, partitions)
        ...
        "matrix_metadata_hash": sha256_payload(
            {
                ...
                "remaining_equations_by_pair": compressed["remaining_equations_by_pair"],
                "equality_pattern_hash": scanner.equality_summary(partitions)["equality_pattern_hash"],
            }
        ),
...
def compute_rank_gates():
    ...
    for candidate in scanner.retained_candidates():
        partitions = candidate["equality_partitions"]
        rows.append(
            {
                "candidate_id": candidate["candidate_id"],
                "equality_pattern_hash": candidate["equality_design"]["equality_pattern_hash"],
                "surrogate": rank_gate(partitions, "surrogate"),
                "exact": rank_gate(partitions, "exact"),
            }
        )
...
def main():
    ...
    for row in record["candidates"]:
        ...
        assert row["equality_design"]["equality_pattern_hash"] == by_id[candidate_id][
            "equality_pattern_hash"
        ]
    ...
    if args.json:
        ...
        print("SAGE_AUDIT_M1_A327_PAIRWISE_DIVISIBILITY_NULLVECTOR_SYSTEM_OK")
