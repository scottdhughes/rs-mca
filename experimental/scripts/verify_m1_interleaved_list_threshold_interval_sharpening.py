#!/usr/bin/env python3
"""Verify the M1 interleaved-list threshold interval-sharpening ledger."""
...
def verify_record(record: dict[str, Any]) -> None:
    ...
    rows = record["interval_agreements"]
    assert rows[0]["agreement"] == 383
    assert rows[-1]["agreement"] == 292
    assert record["first_possible_clear_agreement_by_packing"] == 372
    assert record["first_proved_clear_agreement_by_linear_overlap"] == 292
    assert record["prior_root_pencil_clear_agreement"] == 291
    assert record["improves_prior_root_pencil_clear_by"] == 1
    ...
    row373 = row_by_agreement(rows, 373)
    ...
    row372 = row_by_agreement(rows, 372)
    ...
    row293 = row_by_agreement(rows, 293)
    assert row293["status"] == "PARTIAL"
    assert row293["root_pencil_lower"] == 6
    assert row293["clears_gate_proved"] is False

    row292 = row_by_agreement(rows, 292)
    assert row292["status"] == "LOWER_ONLY"
    assert row292["lambda_lower"] == 7
    assert row292["clears_gate_proved"] is True
    assert row292["clears_gate_possible"] is True
    construction = row292["construction"]
    assert construction["common_root_size"] == 254
    assert construction["residual_degree"] == 1
    assert construction["codeword_degree_bound"] == 255
    assert construction["degree_less_than_k"] is True
    assert construction["extra_positions_needed_per_witness"] == 38
    assert construction["overlap_count_by_witness"] == [6, 2, 2, 2, 2, 1, 1]
    assert construction["unique_needed_by_witness"] == [32, 36, 36, 36, 36, 37, 37]
    assert construction["unique_total"] == 250
    assert construction["used_complement_points"] == 258
    assert construction["uses_all_complement_points"] is True
    assert construction["generic_choice_budget"]["capacity_ok"] is True
    assert construction["generic_choice_budget"]["available_after_star_points"] == 252
    assert len(construction["overlap_points"]) == 8
    assert record["proved_clear_row"] == row292
