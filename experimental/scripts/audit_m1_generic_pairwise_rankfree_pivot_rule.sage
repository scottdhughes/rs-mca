#!/usr/bin/env sage
"""Sage audit for the M1 generic-pairwise rank-free pivot-rule packet."""
...
def load_base_audit():
    module = load_module("base_pivot_audit_generic_pairwise", BASE_AUDIT_PATH)
...
def source_candidate_map(base):
    ...
    for packet, path in base.SOURCE_SCANNERS.items():
        module = base.load_module("generic_pairwise_" + packet, path)
...
def source_rows_by_key(base):
    scanner = base.load_module("base_pivot_scanner_generic_pairwise", BASE_SCANNER_PATH)
    return {
        row["source_key"]: row
...
def source_profiles_by_key():
    ...
    return {
        ...
        if row["classification"] == "generic_pairwise_rref_pivot"
    }
...
def compact_certificate(base, M, row_meta, row):
    ...
    assert row_types == {"balanced_or_generic_pairwise_row": raw["matrix_cols"]}
    return {
...
def schedule_rules(row_meta, ncols, witness_dims, pivot_pair_counts):
    ...
    rules = {
        "pair_label_lexicographic_order": first_n(
        ...
        "anchored_pair_proxy_first": first_n(
            sorted(
                indexed,
                key=lambda item: (
                    0 if 1 in pair_tuple(item[1]["pair"]) else 1,
                    pair_tuple(item[1]["pair"]),
                    row_position(item[1]),
                ),
            ),
        ...
        "nonanchored_difference_first": first_n(
            sorted(
                indexed,
                key=lambda item: (
                    0 if 1 not in pair_tuple(item[1]["pair"]) else 1,
                    pair_tuple(item[1]["pair"]),
                    row_position(item[1]),
                ),
            ),
        ...
        "rref_profile_pair_quota_mimic": rref_profile_rows(indexed, ncols, pivot_pair_counts),
    }
    return rules


def rule_attempts(base, M, row_meta, witness_dims, pivot_pair_counts):
    ncols = M.ncols()
    cols = list(range(ncols))
    out = []
    for rule, rows in sorted(
        schedule_rules(row_meta, ncols, witness_dims, pivot_pair_counts).items()
    ):
        assert len(rows) == ncols
        ...
        rule_class = (
            "RREF_MIMIC_RULE"
            if rule == "rref_profile_pair_quota_mimic"
...
def compute_generic_pairwise_rows(record=None):
...
def validate_record(record, computed):
    ...
    assert len(by_key) == 6
    assert record["matrix_count"] == 6
    assert record["rule_summary"]["rules_tested"] == 8
...
def main():
    ...
    if args.compute_only:
        print(json.dumps(compute_generic_pairwise_rows(), indent=2, sort_keys=True))
        return
    record = json.loads(DATA_PATH.read_text())
    computed = compute_generic_pairwise_rows(record)
    validate_record(record, computed)
    if args.json:
        print(json.dumps(computed, indent=2, sort_keys=True))
    else:
        print("SAGE_AUDIT_M1_GENERIC_PAIRWISE_RANKFREE_PIVOT_RULE_OK")
