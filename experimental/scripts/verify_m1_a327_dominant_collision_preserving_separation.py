#!/usr/bin/env python3
"""Verifier for the M1 a=327 dominant-collision separation packet."""
...
ALLOWED_STATUS = {
    ...
    "NONDEGENERATE_LOW_RESCHEDULE",
    "DEGENERATE_HIGH_CAPACITY",
...
def verify(record: dict[str, Any]) -> dict[str, Any]:
    ...
    require(record["source_commit"] == "9bbffb8", "wrong source commit")
    require(record["construction_mode"] == "dominant_collision_preserving_separation", "wrong mode")
    ...
    require(record["baseline"]["best_degenerate_capacity_upper_bound"] == 448, "bad baseline")
    require(record["baseline"]["best_pinned_nondegenerate_capacity_upper_bound"] == 91, "bad pin baseline")
    require(record["proof_status"] in {"CANDIDATE", "EXACT_EXTRACTION_NO_A327"}, "bad proof status")
    skel = record["dominant_collision_skeletons"]
    search = record["separation_search"]
    require(skel["systems_tested"] == len(record["results"]) >= 1, "bad system count")
    require(skel["safe_separation_coordinates"] > 0, "no safe coordinates")
    require(search["pin_sets_tested"] > 0, "no pins")
    ...
    for result in record["results"]:
        require(result["dominant_skeleton"]["base_capacity_upper_bound"] >= TARGET_AGREEMENT, "bad skeleton capacity")
        require(result["dominant_skeleton"]["safe_separation_coordinates"] > 0, "system has no safe coords")
    ...
    require(pin_sets == search["pin_sets_tested"], "pin count mismatch")
    ...
    require(cap_preserving == search["capacity_preserving_nondegenerate_vectors"], "cap-preserving mismatch")
    ...
    return {
        "status": "PASS",
        "systems_tested": skel["systems_tested"],
...
def main() -> None:
    ...
    if args.json:
        ...
        print(
            "PASS: M1 a=327 dominant-collision separation "
