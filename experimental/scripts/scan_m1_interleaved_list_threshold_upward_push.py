#!/usr/bin/env python3
"""Emit the M1 interleaved-list threshold upward-push ledger.

This scan specializes the quotient-fiber list floor to the active row
RS[F_17^32,H,256].  It finds that the quotient map x -> x^32 gives a
board-clearing interleaved-list lower bound at agreement a=320.
"""
...
def quotient_floor_row(fiber_size: int, quotient_subset_size: int) -> dict[str, Any]:
    if N % fiber_size != 0:
        raise ValueError("fiber size must divide N")
    quotient_size = N // fiber_size
    if not 0 <= quotient_subset_size <= quotient_size:
        raise ValueError("invalid quotient subset size")
...
def best_row_for_fiber_size(fiber_size: int) -> dict[str, Any] | None:
    ...
    for quotient_subset_size in range(quotient_dimension, quotient_size + 1):
        ...
        if row["clears_gate"]:
            if best is None or row["agreement"] > best["agreement"]:
                best = row
    return best
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        default="experimental/data/m1_interleaved_list_threshold_upward_push.json",
        type=Path,
    )
