#!/usr/bin/env python3
"""Verify the M1 a=327 nonbasis row dependency ansatz ledger."""
...
DATA_PATH = Path("experimental/data/m1_a327_nonbasis_row_dependency_ansatz.json")
NOTE_PATH = Path("experimental/notes/m1/m1_a327_nonbasis_row_dependency_ansatz.md")
...
def verify() -> dict[str, Any]:
    ...
    require(record["source_commit"] == "3d5efb6", "wrong source commit")
    require(record["mca_counted"] is False, "MCA counted unexpectedly")
    require(REQUIRED_NONCLAIMS.issubset(set(record["not_claimed"])), "missing nonclaims")
    require(record["realization_status"] == "ACTUAL_TEMPLATE_ROWSPACES_ONLY", "wrong realization status")
    require(
        record["proof_status"] == "EXACT_EXTRACTION_NO_A327 / NBDEP_NEAR_KERNEL_ONLY / PARTIAL / EXPERIMENTAL",
        "wrong proof status",
    )

    previous = record["previous_rankdefect_ansatz"]
    require(previous["commit"] == "3d5efb6", "wrong previous commit")
    require(previous["coefficient_kernel_profiles"] == 0, "unexpected previous coefficient kernel")
    require(previous["best_failure_mode"] == "RANKDEFECT_COEFFICIENT_FULL_RANK", "wrong previous failure")
    ...
    require(search["forced_basis_profiles_tested"] == 4260, "wrong forced-basis profile count")
    require(search["row_removal_limit"] == 3, "wrong row-removal limit")
    require(search["actual_kernel_profiles"] == 0, "unexpected actual kernel profile")
    require(search["near_kernel_profiles"] == 252, "wrong near-kernel profile count")
    require(search["remove_limit_full_rank_profiles"] == 4008, "wrong remove-limit full-rank count")
    ...
    require(search["best_failure_mode"] == "NBDEP_NEAR_KERNEL_ONLY", "wrong best failure")
    require(
    ...
    require(best["actual_kernel_profiles"] == 0, "unexpected best actual kernel")
    require(best["near_kernel_profiles"] == 8, "wrong best near-kernel profile count")
    require(best["pair_projection_clear_actual_kernels"] == 0, "unexpected best pair-clear kernel")
...
def main() -> None:
    ...
    if args.json:
        ...
        print(f"PASS: M1 a=327 nonbasis row dependency ansatz (status={result['proof_status']})")
