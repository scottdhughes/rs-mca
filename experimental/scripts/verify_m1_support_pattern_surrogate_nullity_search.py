#!/usr/bin/env python3
"""Verify the M1 support-pattern surrogate-nullity search checkpoint."""
...
DATA_PATH = Path("experimental/data/m1_support_pattern_surrogate_nullity_search.json")
...
def verify_candidate(row: dict[str, Any]) -> None:
    ...
    assert row["mutation_family"] == "balanced_two_switch_membership_hypergraph"
    assert row["seed_design"] in {
        "cyclic_45_interval_high_overlap",
        "cyclic_3456_near_boundary",
    }
    assert row["search_objective"] in {"pair_boundary", "anchor_compression", "fiber_repetition"}
    assert isinstance(row["random_seed"], int)
    assert row["mutation_steps"] == 1200
    assert row["accepted_mutations"] >= 0
    ...
    assert support["pair_intersection_min"] >= 0
    ...
    assert support["pair_intersections_at_255"] >= 3
    ...
    assert support["membership_pattern_count"] >= 15
...
def verify_record(record: dict[str, Any]) -> None:
    ...
    assert summary["generated_candidate_count"] == 12
    assert summary["retained_candidate_count"] == 6
    assert summary["exact_audited_count"] == 6
    assert summary["candidate_family"] == "surrogate-scored balanced two-switch membership mutations"
    ...
    assert record["sage_audit"]["script"] == (
        "experimental/scripts/audit_m1_support_pattern_surrogate_nullity_search.sage"
    )
    assert record["sage_audit"]["recomputes_retained_candidates"] is True
    assert record["sage_audit"]["constructs_GF_17_32"] is True
    assert record["sage_audit"]["uses_surrogate_rank_gate"] is True
    assert record["sage_audit"]["uses_exact_rank_gate"] is True
    ...
    assert record["status"] == "M1_SUPPORT_PATTERN_SURROGATE_NULLITY_SEARCH_ROUTE_CUT_PARTIAL"
...
def main() -> None:
    ...
    if args.json:
        ...
        print("PASS")
        print("six surrogate-scored support-pattern mutations exact-audited")
