#!/usr/bin/env sage
"""Exact GF(17^32) audit for the random-matroid functional lift."""
...
def solve_functional_coordinates(F, row, basis_functionals):
    d = len(basis_functionals)
    matrix = Matrix(F, [[F(basis_functionals[j][col]) for j in range(d)] for col in range(d)])
    rhs = vector(F, [F(value) for value in row])
    return list(matrix.solve_right(rhs))
...
def audit(sample_limit=16, rank_only=False):
    ...
    if not record["basis_profiles"]:
        record["functional_lift"]["best_failure_mode"] = "RANDOM_MATROID_FUNC_LIFT_NO_BASIS"
        record["proof_status"] = "EXACT_EXTRACTION_NO_A327 / RANDOM_MATROID_FUNC_LIFT_NO_BASIS / PARTIAL / EXPERIMENTAL"
        return record
    ...
    record["functional_lift"].update(
        {
            ...
            "H_order": len(H),
            "best_failure_mode": "RANDOM_MATROID_FUNC_LIFT_NULLITY_POSITIVE" if nullity > 0 else "RANDOM_MATROID_FUNC_LIFT_NULLITY_ZERO",
        }
    )
    record["proof_status"] = (
        "CANDIDATE / RANDOM_MATROID_FUNC_LIFT_NULLITY_POSITIVE / PARTIAL / EXPERIMENTAL"
        if nullity > 0
        else "EXACT_EXTRACTION_NO_A327 / RANDOM_MATROID_FUNC_LIFT_NULLITY_ZERO / PARTIAL / EXPERIMENTAL"
    )
    ...
    if projection["forced_equal_pairs"]:
        record["functional_lift"]["best_failure_mode"] = "RANDOM_MATROID_FUNC_LIFT_FORCED_PAIR_EQUALITY"
        record["proof_status"] = "EXACT_EXTRACTION_NO_A327 / RANDOM_MATROID_FUNC_LIFT_FORCED_PAIR_EQUALITY / PARTIAL / EXPERIMENTAL"
        return record
    record["functional_lift"]["best_failure_mode"] = "RANDOM_MATROID_FUNC_LIFT_PAIR_PROJECTIONS_CLEAR"
    record["proof_status"] = "CANDIDATE / RANDOM_MATROID_FUNC_LIFT_PAIR_PROJECTIONS_CLEAR / PARTIAL / EXPERIMENTAL"
    for vector_row in deterministic_candidates(F, basis, sample_limit):
        ...
        if (
            ...
            record["functional_lift"]["best_failure_mode"] = "RANDOM_MATROID_FUNC_LIFT_EXACT_CANDIDATE"
            record["proof_status"] = "PROOF_RECORD / RANDOM_MATROID_FUNC_LIFT_EXACT_CANDIDATE / EXPERIMENTAL"
            return record
    record["functional_lift"]["best_failure_mode"] = "RANDOM_MATROID_FUNC_LIFT_DEGENERATE_SAMPLE"
    return record
...
def main():
    ...
    if args.json:
        ...
        print("SAGE_AUDIT_M1_A327_RANDOM_MATROID_FUNCTIONAL_LIFT_OK")
        print("best_matrix_shape: %s" % lift["best_matrix_shape"])
        print("best_rank: %s" % lift["best_rank"])
        print("best_nullity: %s" % lift["best_nullity"])
        print("best_failure_mode: %s" % lift["best_failure_mode"])
