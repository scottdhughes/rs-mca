#!/usr/bin/env python3
"""Verifier for the M1 a=327 joint target/codeword solver."""
...
SCAN_DATA_PATH = Path("experimental/data/m1_a327_joint_target_codeword_solver.json")
EXACT_DATA_PATH = Path("experimental/data/m1_a327_joint_target_codeword_solver_exact_audit.json")
...
def verify_scan(record: dict[str, Any]) -> None:
    ...
    require(record["construction_mode"] == "joint_target_codeword_solver", "wrong construction mode")
    require(record["model"]["anchor"] == "P_1 = 0", "wrong anchor")
    require(record["model"]["variables"] == "six degree<256 difference polynomials", "wrong variable model")
    require(record["model"]["proxy_field"] == "GF(12289)", "wrong proxy field")
    require(record["model"]["row_budget"] == 384, "wrong row budget")
    require(record["model"]["samples_per_system"] == 8, "wrong sample count")
    require(len(record["source_embedding_ids"]) == 8, "wrong source embedding count")
    require(len(record["target_strategies"]) == 5, "wrong strategy count")
    require(record["target_system_count"] == 40, "wrong target system count")
    require(record["codeword_tuple_sample_count"] == 320, "wrong codeword sample count")
    require(record["proxy_candidate_system_count"] == 0, "unexpected proxy candidate system")
    ...
    require(record["best"]["target_system_id"] == "all_pair_boundary_fiber_round_robin__quotient_fiber_round", "wrong best system")
    require(record["best"]["best"]["capacity_upper_bound"] == 454, "unexpected best capacity")
    require(best_assignment_floor(record["best"]) == 311, "unexpected best assignment floor")
    for row in record["retained_results"]:
        require(row["proxy_candidate_count"] == 0, "retained row has proxy candidate")
        require(row["sample_count"] == 8, "wrong retained sample count")
        require(row["rank"] <= row["target_row_count"], "rank exceeds row count")
        for sample in row["sample_results"]:
            require(sample["status"] == "CAPACITY_BELOW_A327", "unexpected sample status")
...
def verify_exact(scan: dict[str, Any], exact: dict[str, Any]) -> None:
    ...
    require(exact["construction_mode"] == "joint_target_codeword_solver_exact_audit", "exact wrong mode")
...
def main() -> None:
    ...
    if args.json:
        ...
        print(
            "PASS: M1 a=327 joint target/codeword solver "
