...
- **Agent/model:** Codex, with an independent read-only recovery of the
  deployed source, selector, regular-chart, and rich-pencil interfaces.
...
- **Status:** PROVED executable readiness audit and exact toy incidence kernel /
  AUDIT contract manifest and route cut / deployed incidence theorem OPEN /
  no ledger movement.
- **What is being added:** A fail-closed contract freezes the same-selector
  source, support, regular-chart, graph-line, determinant-mass, and coverage
  records required by the existing rank-nine rich-pencil compiler.  The exact
  checker reconstructs two nonempty small-prime declared-family fixtures plus
  an empty-family edge control and proves their direct fixed-basis excess
  equals their canonical atlas excess, while
  the bound deployed artifacts supply no paying-selector coverage of every
  source family by construction, uniform theorem, or exhaustive census.  The
  toy kernel is not a full deployed-record validator.
- **How it is useful:** It turns the live `UNPAID_SOURCE_BOUND_RICH_PENCIL_AGGREGATE`
  gap into an executable readiness boundary and prevents support moments, one
  supplied selector, or toy declared-family data from being mistaken
  for deployed source incidence.  The current terminal is
  `UNBOUND_DEPLOYED_SOURCE_INCIDENCE`.
- **What to do next:** Derive one row-uniform determinant-weighted incidence
  lemma over the whole declared branch-3 rank-nine successor, preserving the
  pointwise source map and same-selector provenance.  Do not move the ledger,
...
- **Agent/model:** Codex with independent complete-selector and quantifier
  audits.
- **Files added or changed:**
  `experimental/notes/m1/m1_kb_branch3_rank9_cyclic_rich_pencil_control_v1.md`,
  `experimental/data/certificates/m1-kb-branch3-rank9-cyclic-rich-pencil-control-v1/`,
...
- **Status:** PROVED exact cyclic toy closure / PROVED incomplete rank-nine
  atlas control / deployed implication OPEN / no ledger movement.
- **What is being added:** An exact `GF(67^2)` order-34 RS control.  One
  incomplete 29-slope selector has affine rank nine and atlas excess 165, but
  a separate lexicographic selector exhausts all 66 noncontained slopes with
  exact minimum affine rank two and exact minimum carrier excess one.  The
  latter is paid by `CERTIFIED_LOW_EXCESS_COMMON_CARRIER`, with `B_1=231`.
- **How it is useful:** It closes the smallest cyclic toy requested by the
  rich-pencil route and demonstrates why incomplete hostile selectors cannot
  be promoted before optimizing over complete selectors.  It also freezes
  the distinct selected-support and existential periodic/Q0 projections.
- **What to do next:** Use the same complete-selector optimization on a
  source-faithful base-domain or deployed-scale control.  Do not infer a
  deployed rank-nine payment, move the ledger, or start rank at least ten.

### 2026-07-17 - KoalaBear rank-nine rich-pencil aggregate route cut

- **Agent/model:** Codex with independent next-lemma and source-contract
  audits.
- **Files added or changed:**
  ...
  and experimental/agents-log.md.
- **Status:** PROVED canonical compiler / exact scalar route cut / exact
  generic-local control / deployed rich-pencil incidence OPEN / no ledger
  movement.
- **What is being added:** An exact decomposition of the fixed-basis excess
  into canonically deduplicated affine graph-line/codeword-pencil components,
  plus a deployed-integer \(x=1,J=109\) relaxation showing the existing local
  constraints still permit one component to exceed the aggregate allowance.
  An exact \(\mathbb F_{2^{37}}\) replay verifies direct excess equals the
  five-line atlas sum \(817\).
- **How it is useful:** It replaces overlapping basis fibres by the precise
  weighted rich-pencil incidence theorem the sparse rank-nine route needs,
  identifies the forced common GCD and source-bound plant, and prevents the
  incomplete branches 4--5 interface from being treated as a paid residual.
- **What to do next:** Search exact cyclic \(\mathbb F_{p^6}/\mathbb F_p\)
  controls for a rich pencil plus eight rank-nine outlier directions.  Stop
  at the first source-bound countertemplate; otherwise freeze the surviving
  determinant/GCD support equations.  Do not move the ledger, start rank at
  least ten, determine \(U_Q/U_A\), or use Lean.

### 2026-07-17 - KoalaBear rank-nine fixed-basis fibre route cut

- **Agent/model:** Codex with independent proof-audit and literature-search
  subagents.
- **Files added or changed:**
  `experimental/notes/m1/m1_kb_branch3_rank9_fixed_basis_fibre_route_cut_v1.md`,
  `experimental/data/certificates/m1-kb-branch3-rank9-fixed-basis-fibre-v1/`,
  ...
  and `experimental/agents-log.md`.
- **Status:** PROVED / COUNTEREXAMPLE / ROUTE CUT / AGGREGATE INPUT OPEN /
  NO LEDGER MOVEMENT.
- **What is being added:** A proof that every fixed rank-eight kernel-basis
  fibre is one affine word line, the sharp union-support threshold
  `M>=1030160` forcing fibre multiplicity at most 20, and the exact aggregate
  excess allowance that would pay the deployed `H_18014` tail.  An exact
  `GF(2^37)`, `j=20` five-pencil family has rank tuple `(9,10,11)` and 21
  all-zero-deficit masks through one fixed basis, refuting the proposed
  uniform cap 20.
- **How it is useful:** It prevents another false pointwise incidence lemma
  while reducing the surviving M2b route to one precise aggregate excess
  statement.  It also identifies the low-union fixed-basis affine-line cell
  that must be coupled across bases or handled by deployed first-match
  exhaustion.
- **What to do next:** Independently review and bank the route cut.  Then
  attack the aggregate excess bound on the source-bound sparse regular route,
  or construct a deployed countertemplate exceeding it.  Do not sum global
