...
### 2026-07-04 - M1 a327 pair-clear nine-row module syzygy

- **Agent/model:** Codex.
- **Files added or changed:**
  `experimental/notes/m1/m1_a327_pairclear_nine_row_module_syzygy.md`,
  `experimental/scripts/scan_m1_a327_pairclear_nine_row_module_syzygy.py`,
  `experimental/scripts/verify_m1_a327_pairclear_nine_row_module_syzygy.py`,
  `experimental/scripts/m2_m1_a327_pairclear_nine_row_module_syzygy.m2`,
  `experimental/data/m1_a327_pairclear_nine_row_module_syzygy.json`,
  `experimental/agents-log.md`.
- **Status:** AUDIT / NINEROW_MODULE_SYZYGY_STABLE / PARTIAL / EXPERIMENTAL.
- **What is being added:** A Python + Macaulay2 module audit for the pinned
  nine-row pair-clear direction system. It splits the 14 coefficient rows into
  five inactive rows and nine active rows for direction `[0,5,0,0,0,1]`.
- **Result:** Python over `GF(17)` gives full matrix rank 6, active matrix rank
  6, and inactive matrix rank 5 with one-dimensional right kernel generated by
  `[0,5,0,0,0,1]`. Adding any one of the nine active rows to the inactive
  matrix gives rank 6. Macaulay2 independently confirms full rank 6, active
  rank 6, inactive rank 5, one inactive right-kernel generator, and rank 6 for
  all nine singleton extensions.
- **How it is useful:** Certifies that inside the current inactive-row chamber,
  the pinned direction is unique up to scalar and every active row destroys that
  kernel. This explains why the local direction scan stayed at nine rows.
- **What to do next:** Search for a different pair-clear direction chamber
...
### 2026-07-04 - M1 a327 pair-clear two-target basis exchange search

- **Agent/model:** Codex.
- **Files added or changed:**
  `experimental/notes/m1/m1_a327_pairclear_two_target_basis_exchange_search.md`,
  `experimental/scripts/scan_m1_a327_pairclear_two_target_basis_exchange_search.py`,
  `experimental/scripts/verify_m1_a327_pairclear_two_target_basis_exchange_search.py`,
  `experimental/data/m1_a327_pairclear_two_target_basis_exchange_search.json`,
  `experimental/agents-log.md`.
- **Status:** EXACT_EXTRACTION_NO_A327 /
  TWOTARGETBASIS_FORCED_PROJECTIONS_REMAIN / PARTIAL / EXPERIMENTAL.
- **What is being added:** A bounded basis-model expansion allowing exactly
  two target zero classes into the coefficient basis and imposing both zero
  conditions as unit coordinate rows.
- **Result:** The scan tested 13,700 basis profiles and found 640 extended
  rank-slack profiles, up from 512 in the one-target exchange. It still found
  zero exact pair-clear and zero cycle-pressure-reduced profiles. The best
  profile, `ninerow_P17_shear_c1_d1` with basis
  `twotargetbasis_6_7_0_1_2_4_6_7`, has target basis classes `[6,7]` and
  extended rank 4/nullity 2. Its best direction `[0,0,1,2,0,0]` still forces
  `[P45,P46,P56]`.
- **How it is useful:** Shows that widening target-basis exchange expands the
  rank-slack front but still does not reduce pair-clear cycle pressure.
