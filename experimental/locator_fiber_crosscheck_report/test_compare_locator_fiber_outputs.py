...
def test_fail_on_unmatched_exits_nonzero_for_one_sided_cases(tmp_path):
    python_csv = tmp_path / "locator_fiber_sweep.csv"
    sage_json = tmp_path / "sage_locator.json"
    out_dir = tmp_path / "report"
    write_python_csv(python_csv)
...
def test_fail_on_unmatched_allows_complete_comparison(tmp_path):
    python_csv = tmp_path / "locator_fiber_sweep.csv"
    sage_json_one = tmp_path / "sage_one.json"
    sage_json_two = tmp_path / "sage_two.json"
    out_dir = tmp_path / "report"
    ...
    assert exit_code == 0
    report = read_json(out_dir / "locator_fiber_crosscheck_report.json")
    assert report["summary"]["python_only_cases"] == 0
    assert report["summary"]["sage_only_cases"] == 0
