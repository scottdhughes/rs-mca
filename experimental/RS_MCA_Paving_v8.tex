...
\title{MDS Paving Bounds for Reed--Solomon MCA}
...
\hypersetup{pdftitle={MDS Paving Bounds for Reed--Solomon MCA},
pdfauthor={Przemek Chojecki},
pdfsubject={Finite and asymptotic bounds for Reed--Solomon mutual correlated agreement},
pdfkeywords={Reed-Solomon codes, mutual correlated agreement, Proximity Prize, MDS codes, paving matroids, circle codes}}
\subjclass[2020]{94B35, 11T71, 05B40}
\keywords{Reed--Solomon codes, mutual correlated agreement, Proximity Prize,
MDS codes, paving matroids, soundness gaps, circle codes}
\date{July 16, 2026}
...
\begin{abstract}
We prove an integrated support-count and paving-information-set upper bound for affine support-wise mutual correlated agreement of every linear MDS code.  It yields finite smooth multiplicative and explicitly monomially equivalent circle Reed--Solomon certificates with \(\varepsilon_{\rm mca}\le2^{-128}\) beyond the Johnson radius.  Using an exact CA--MCA decomposition and a prior BCHKS proximity-gap bound, we give challenge-restricted, endpoint-exact linear-budget thresholds through asymptotically half the minimum distance.  For exponential budgets, the Johnson-range estimate and the MDS envelope give an explicit one-sided safe-frontier certificate.  We distinguish the customary subcapacity threshold from the official radius interval \([0,1]\), and prove all-radius saturation once the integer slope budget dominates \(\binom n{k+1}\).  Finally, we isolate a conditional retained-interpolation calculation and prove that an image-normalized Sidon payment at an accessible moment order implies primitive max-fiber flatness; the missing analytic payment and ray compiler leave the unrestricted constant-rate smooth/circle frontier open.
\end{abstract}
...
but is not load-bearing here.  Below half
...
phenomenon.  Our additional universal mechanism is an augmented-word
paving-matroid double count: it applies to every MDS code at every agreement
and gives finite smooth and circle certificates beyond Johnson.  A separate
...
and a one-sided exponential-budget safe-frontier certificate.
...
\midrule
Integrated support/paving MDS envelope & proved here &
The augmented-word paving double count and its intersection with the exact
support count are the principal proposed new contribution. \\
...
Exponential-budget safe frontier & proved integration here & The Johnson
input and the MDS envelope give a one-sided certificate, not a matching
...
The exact linear-budget theorem, the MDS paving envelope, the one-sided
exponential-budget certificate, the all-radius completion, and every
explicitly verified finite paving row are unconditional subject only to the
displayed imported literature theorems.  The parameter-retained appendix is
...
release as \path{AI_USE_v8.md}.

\begin{theorem}[Integrated MDS envelope]\label{thm:intro-paving}
...
\[
 ...
 \le U_{n,k,\Gamma}(a)
 \defeq\min\left\{\abs\Gamma,\binom na,
 \floor{\frac{\binom n{k+1}}{\binom{a-1}k}}\right\}.       \tag{1.3}
...
\begin{corollary}[Four official-rate budget-saturated rows]
...
The two terms after \(\abs\Gamma\) are complementary.  The support atlas is
exact at \(a=n\), whereas the paving term is strongest in much of the
interior.  For Reed--Solomon rows and their monomially equivalent GRS or
...
For exponentially large budgets define, for \(0\le\delta<1-\rho\),
\[
 \Phi_\rho(\delta)=H_2(\rho)
 -(1-\delta)H_2\!\left(\frac{\rho}{1-\delta}\right),\quad
 J_\rho=1-\sqrt\rho,\quad
 \theta_J=\Phi_\rho(J_\rho)
 =H_2(\rho)-\sqrt\rho\,H_2(\sqrt\rho).             \tag{1.8}
 \label{eq:intro-Phi}
\]
The function \(\Phi_\rho\) is continuous and strictly increasing from
zero to \(H_2(\rho)\), since
\(\Phi_\rho'(\delta)=
\log_2((1-\delta)/(1-\delta-\rho))>0\).  Put
\[
 D_\rho^{\rm sub}(\theta)=
 \begin{cases}
 J_\rho,&0<\theta\le\theta_J,\\
 \Phi_\rho^{-1}(\theta),&\theta_J<\theta<H_2(\rho),\\
 1-\rho,&\theta\ge H_2(\rho).
 \end{cases}                                      \tag{1.9}
 \label{eq:intro-D}
\]
and define the official certificate
\[
 D_\rho^{\rm off}(\theta)=
 \begin{cases}
 D_\rho^{\rm sub}(\theta),&0<\theta\le H_2(\rho),\\
 1,&\theta>H_2(\rho).
 \end{cases}                                      \tag{1.10}
 \label{eq:intro-D-official}
\]

\begin{theorem}[Exponential-budget safe-frontier certificate]
...
For every fixed \(0<\delta<J_\rho\), put
\(\rho_n^-=(k_n-1)/n\),
\[
 m_n=\max\left\{\left\lceil
 \frac{\sqrt{\rho_n^-}}{1-\sqrt{\rho_n^-}-\delta}
 \right\rceil,3\right\},\qquad t_n=m_n+\frac12.
\]
BCHKS \cite[Thm.~4.6]{BCHKS25}, with its parameter \(M=1\) and
degree-at-most parameter \(k_n-1\), gives
\[
 B_{C_n}^{\rm MCA}(n-\floor{\delta n})
 \le
 \frac{2t_n^5+3t_n\delta\rho_n^-}
      {3(\rho_n^-)^{3/2}}\,n
 +\frac{t_n}{\sqrt{\rho_n^-}}.                   \tag{1.13}
 \label{eq:intro-johnson-import}
\]
For large \(n\), the denominator in the definition of \(m_n\) stays
positive and \(m_n=O_{\rho,\delta}(1)\), so the displayed numerator is
linear in \(n\).  At \(\delta=0\), the same subexponential conclusion
...
is eventually below a budget of positive exponent.  For
\(J_\rho\le\delta<1-\rho\), take
\(a_n=n-\floor{\delta n}\) in \cref{thm:intro-paving}.  Stirling's
formula gives
\[
 \frac1n\log_2
 \frac{\binom n{k_n+1}}{\binom{a_n-1}{k_n}}
 =\Phi_\rho(\delta)+o(1).                          \tag{1.14}
 \label{eq:intro-paving-stirling}
\]
Hence every fixed radius with \(\Phi_\rho(\delta)<\theta\) is safe.
Taking the union of the Johnson and paving ranges gives
\eqref{eq:intro-D}--\eqref{eq:intro-certified-frontier}; the official
...
The only input to the preceding proof that is not proved in this paper is
the stated BCHKS Theorem~4.6.  Its source gives the displayed theorem and
then sketches the list-correlated-agreement generalization.  Thus
\eqref{eq:intro-johnson-import} and the below-Johnson part of
\cref{thm:intro-exponential-frontier,cor:intro-security-exponents} depend on
that published statement; the self-contained MDS envelope and its finite
certificates do not.
\end{remark}
...
\[
 ...
 \ge\bigl(\phi-\Phi_\rho(\delta)\bigr)_+
...
\(\min\{\phi,\Phi_\rho(\delta)\}\).  For a target
\(2^{-\lambda_n}\) with \(\lambda_n\ge0\),
\(\lambda_n/n\to\lambda\), and
\(\phi-\lambda>0\), the certified limiting frontier is
\(D_\rho^{\rm sub}(\phi-\lambda)\): every fixed radius strictly below it is
...
lower numerator have zero base-two exponent, which gives the limit.  Beyond
Johnson, combine the field cap with
\eqref{eq:intro-paving-stirling}.  The target statement follows because
...
\begin{lemma}[Bases in a paving matroid]\label{lem:paving-bases}
If \(M\) is a rank-\(s\) paving matroid on \(v\ge s\) elements, then
\[
                         b(M)\ge\binom{v-1}{s-1},  \tag{2.3}
 \label{eq:paving-bases}
\]
where \(b(M)\) denotes the number of bases of \(M\).
The bound is sharp, for example for the direct sum of a coloop and
\(U_{s-1,v-1}\).  This classical estimate also follows from the
\(h\)-vector description in \cite[Prop.~3.1]{MNRI11}.
\end{lemma}

\begin{proof}
Induct on \(v+s\).  If \(s=1\), a rank-one matroid has a nonloop and
therefore at least one basis, which is the required bound.  If \(v=s\),
the whole ground set is the unique basis.  Now assume \(1<s<v\).
A rank-\(s\) paving matroid has no loops, and at most \(s\) elements can
be coloops, so choose a non-coloop \(e\).  Then \(M\setminus e\) and
\(M/e\) are paving of ranks \(s\) and \(s-1\), respectively.  Bases
avoiding and containing \(e\) are counted by these two minors, so
induction and Pascal's identity give
\(b(M)\ge\binom{v-2}{s-1}+\binom{v-2}{s-2}
=\binom{v-1}{s-1}\).
\end{proof}

\begin{lemma}[Exact MDS witness]\label{lem:exact-mds-witness}
Let \(C\subseteq\F^n\) be a linear \([n,k]\) MDS code and \(a\ge k+1\).
Every threshold-\(a\) MCA witness contains an exact-\(a\) witness for the
same received pair and slope.
\end{lemma}

\begin{proof}
Let \(S\) be a witnessing support and choose a coordinate word \(u_i\)
whose restriction is not in \(C|_S\).  On a fixed \(k\)-subset
\(K\subseteq S\), the MDS property gives a unique punctured codeword
agreeing with \(u_i|_K\).  It disagrees at some \(x\in S\setminus K\);
otherwise it would explain \(u_i\) on \(S\).  Thus
\(u_i|_{K\cup\{x\}}\notin C|_{K\cup\{x\}}\).  Extend this set inside
\(S\) to size \(a\).  Nonmembership persists, and the original affine
combination remains explained.
\end{proof}

\begin{proof}[Proof of \cref{thm:intro-paving}]
The CA inequality is immediate because column-far pairs form a subclass
of the received pairs in the MCA maximum.  By
\cref{lem:exact-mds-witness}, choose an exact \(a\)-support for each bad
slope.  A noncommon support carries at most one slope, so the chosen-support
map is injective and gives the atlas bound \(\binom na\).

It remains to prove the paving term.  Fix a received pair \(u=(u_0,u_1)\)
and let \(Z\subseteq\Gamma\) be its bad-slope set.  For every
\(\gamma\in Z\), choose an exact witness \(S_\gamma\), an explaining
codeword there, and \(i(\gamma)\in\{0,1\}\) such that
\(u_{i(\gamma)}|_{S_\gamma}\notin C|_{S_\gamma}\).
Append that word as a last row to a \(k\)-row generator matrix of
\(C|_{S_\gamma}\).  The augmented matrix has rank \(k+1\), and each set
of at most \(k\) columns is independent because the original matrix
generates an MDS code.  Its column matroid is therefore rank \(k+1\)
paving.  By \cref{lem:paving-bases}, at least \(\binom{a-1}k\) of its
\((k+1)\)-subsets are bases.

For each such subset \(T\), let \(\ell_T\ne0\) be a parity functional
with kernel \(C|_T\).  The explaining condition gives
\[
 \ell_T(u_0|_T)+\gamma\ell_T(u_1|_T)=0.            \tag{2.4}
 \label{eq:paving-affine-equation}
\]
At least one coefficient is nonzero because \(T\) is an augmented basis;
since the equation holds for the chosen \(\gamma\), it determines at most
one finite slope.  Double counting the pairs \((\gamma,T)\) yields
\[
 \abs Z\binom{a-1}k\le\binom n{k+1}.
\]
Taking the integer floor, maximizing over received pairs, and intersecting
the three applicable caps proves \eqref{eq:intro-paving}.  Coordinate
permutations and invertible diagonal scalings preserve punctured
membership and the affine equation, which proves the invariance statement.
\end{proof}

\begin{remark}[The actual paving gain]\label{rem:paving-saving}
Ignoring challenge caps, the paving term improves the atlas only when its
ratio is smaller.  In that range the bit gain is
\[
 \log_2\!\left(
 \frac{\binom na\binom{a-1}k}{\binom n{k+1}}\right)
 =\log_2\!\left(\frac{k+1}{a}
                 \binom{n-k-1}{a-k-1}\right).     \tag{2.5}
\]
Writing \(U_{\rm atlas}=\min\{\abs\Gamma,\binom na\}\), globally the gain is
\(\log_2(U_{\rm atlas}/U_{n,k,\Gamma})\ge0\), which correctly vanishes
near \(a=n\).  The local multiplicity \(\binom{a-1}k\) is best possible
when a word differs from a punctured codeword in one coordinate; a
stronger universal theorem must use compatibility among different slopes.
\end{remark}


\begin{proposition}[Two finite rows beyond Johnson]
...
prime.  At target \(2^{-128}\), the integrated MDS envelope certifies
\[
\begin{array}{c|c|c|c|c}
 k&a&(512-a)/512&1-\sqrt{k/512}&U_{512,k,\F_p}(a)\\ \hline
 32&52&115/128&3/4&
 210954686508560867421211382134708972291\\
 64&158&177/256&1-1/\sqrt8&
 275511258760747555342982982548156580976 .
\end{array}                                       \tag{2.6}
 \label{eq:paving-finite-rows}
\]
Both upper numerators are at most \(B\); hence both radii are safe and
lie strictly beyond Johnson.  They are exact evaluations of the proved
upper bound, not claims that the MCA numerator attains them, and failure
of the same certificate one agreement lower would not prove unsafety.
\end{proposition}

\begin{proof}
The two entries are the integer divisions
\(\floor{\binom{512}{33}/\binom{51}{32}}\) and
\(\floor{\binom{512}{65}/\binom{157}{64}}\).
...
\(512\mid p-1\) gives the subgroup.  The exact divisions are at most
\(B\), and the radius comparisons are
\(115/128>3/4\) and \(177/256>1-1/\sqrt8\).
\end{proof}
...
\]
If \(R\ge2\), \(0\le\eps\le1\), and
\(b=\floor{\eps\abs\F}\), define
\[
 a^*=\min\{a\in\{k+1,\ldots,n\}:B_C^{\rm MCA}(a)\le b\},
\]
whenever this set is nonempty.  The first safe nontrivial agreement then
satisfies
\[
 \begin{array}{ll}
...
\end{array}                                       \tag{AD2}
\]
In the first branch the official safe set is \([0,1]\) and
\(\delta_{C,\mathrm{off,grid}}^*
=\delta_{C,\mathrm{off,sup}}^*=1\); in the second branch the transition
occurs before capacity.
...
\[
 ...
 U(a)&=\min\left\{\abs\Gamma,\binom na,
       \floor{\frac{\binom n{k+1}}{\binom{a-1}k}}\right\}.
 \end{split}                                       \tag{SB1}
\]
...
\cref{prop:simple-pole-lower}.  The integrated MDS envelope gives
...
exponential-budget regime, the Johnson theorem and the integrated MDS
envelope give the explicit subcapacity certificate
\(D_\rho^{\rm sub}(\theta)\).  For
...
The augmented-word paving count is the paper's additional universal mechanism;
we are unaware of this application in the proximity-gap literature.
Intersecting it with the exact support atlas is essential and yields a
finite security interval rather than a one-sided score.  It gives
multiplicative and circle certificates beyond Johnson at length \(512\).
...
choosing \(\sigma<\eta/(4C_0)\).  The payment and residual ray compiler remain substantive row
inputs.
...
The paving bound is strongest in short, low-rate rows over very large
...
determinations and not beyond-Johnson certificates.  The paving theorem
...
\path{verify_paving_mca_v8.py} checks the
unconditional paving, all-radius saturation, circle, primality,
field-budget, and Johnson
comparisons, while \path{verify_retained_bchks_v8.py} checks only the
...
\path{REPRODUCIBILITY_v8.md}.  General proof-development materials are
pinned at
...
\newblock \url{https://github.com/przchojecki/rs-mca}, accessed July 16, 2026.
...
\newblock \url{https://proximityprize.org/}, accessed July 16, 2026.
...
\bibitem[MNRI11]{MNRI11}
C. Merino, S. D. Noble, M. Ram\'irez-Ib\'a\~nez, and R. Villarroel,
\newblock On the structure of the \(h\)-vector of a paving matroid,
\newblock \emph{European J. Combin.} 33 (2012), no.~8, 1787--1799;
\newblock arXiv:1008.2031.
