...
\;\ge\;2^{-86}\;\gg\;2^{-128}
\]
for every $\delta\in[1-\rho-2^{-9},1-\rho)$ at rates $1/2,1/4,1/8$, and for every $\delta\in[1-\rho-2^{-10},1-\rho)$ at rate $1/16$. The same lower bound holds for $\eca(C,\delta)$ on the CS-admissible subinterval ending at $1-\rho-1/n$. If $|\F|\ge2n$, the displayed lower bound improves to $2^{-42}$. Consequently $\delta^*_C(2^{-128})\le 1-\rho-2^{-9}$ for $\rho\in\{1/2,1/4,1/8\}$ and $\delta^*_C(2^{-128})\le 1-\rho-2^{-10}$ for $\rho=1/16$, throughout the $|\F|<2^{256}$ challenge envelope.

The mechanism composes a pigeonhole lower bound on quotient-locator list fibers---a slack-two and subfield-pigeonhole sharpening of \cite[Main Thm.\,(d)]{Cho26a}---with the correlated-agreement-to-list-decoding theorem of Crites--Stewart \cite{CS25}, used directly in the integer-radius form implied by \cite[Thm.~2]{CS25}. The composition itself was first carried out, conditionally and with weaker constants, in the universal-cap theorem of \cite{Cho26b} (error $2^{-42}$ at gap $2^{-11}$, assuming $|\F|\ge2n$); the present note sharpens the gap, removes the field-size condition, and extends the cap to extension fields. No value-set or equidistribution input is used, so the per-fiber collision problem isolated in \cite{Cho26b} is routed around rather than solved. We instantiate the bound at the KoalaBear-sextic parameters of \cite[\S6.3]{ABF26}, including the interleaved rows of their Tables~2--3, give an independent slacked variant through \cite[Thm.~1.9]{BCHKS25}, and prove a two-radius refinement of the subfield-confinement theorem of \cite{Cho26b}, showing that over extension fields every explicit base-rational locator line is inert: the CS25-certified MCA failure must be witnessed by genuinely $\F$-valued lines.
\end{abstract}
...
\paragraph{Imported theorems.} The main composition below uses Crites--Stewart directly, with its integer-radius admissibility condition; see \cref{rem:import}. We also keep the slacked BCHKS/ABF fallback as a separate imported route.

\begin{theorem}[Crites--Stewart conversion, relative-radius form]\label{thm:A}
Let $C=\RS[\F,D,k]$, let $C^+:=\RS[\F,D,k+1]$, let $q:=|\F|$ and $n:=|D|$. Let $\delta\in(0,1)$ and set $f_\delta:=\lfloor \delta n\rfloor$. Assume
\[
        f_\delta < n-k-1 .
\]
...
\begin{proof}
Let $\epsilon:=\eca(C,\delta)$. Since Hamming distances are integral, the event $\dist(\cdot,C)\le\delta$ is the same as having Hamming distance at most $f_\delta=\lfloor\delta n\rfloor$. Likewise the ``far'' condition in the no-proximity-loss correlated-agreement definition is the corresponding integer condition at radius $f_\delta$.

Theorem~2 of Crites--Stewart~\cite{CS25} says that if $\RS(\F,D,k)$ satisfies correlated agreement over lines with $f<n-k-1$ errors and error parameter $\epsilon<(q-n)/(kq)$, then $\RS(\F,D,k+1)$ is $(f/n,L)$-list decodable with
\[
        L
        =
        \left\lceil
...
\Cref{thm:A} is now used directly in the form implied by Crites--Stewart~\cite[Thm.~2]{CS25}, rather than through the ABF26 restatement. The important admissibility condition is the integer-radius condition
\[
...
\]
Moreover, the same lower bound holds for $\eca(C,\delta)$ throughout the CS-admissible subinterval
\[
        \delta\in
...
\]
If $q\ge2n$ then $1-n/q\ge1/2$, giving the sharper $1/(4k)\ge2^{-42}$. The gap is $2/N_\rho=2^{-9}$ for $\rho\in\{1/2,1/4,1/8\}$ and $2/N_\rho=2^{-10}$ for $\rho=1/16$. \Cref{thm:main} gives the MCA lower bound throughout the stated sub-capacity interval, and gives the CA lower bound on the CS-admissible subinterval ending at $1-\rho-1/n$.
\end{proof}
...
\]
Moreover, the same lower bound holds for $\eca$ throughout the CS-admissible subinterval
\[
        \delta\in
...
\]
Moreover, the same lower bound holds for $\eca(C_s^{\equiv s},\delta)$ on the CS-admissible subinterval
\[
        \delta\in
...
\]
exceeds $2^{-154}$ contains a word that is not $\B$-valued, even after removing a common scalar factor. In particular, any pair attaining the MCA lower bound $>2^{-22}$ of \cref{cor:deployed} is genuinely $\F$-valued. On the CS-admissible subinterval
\[
        \delta\in
        \Bigl[\frac12-2^{-7},\,\frac12-2^{-21}\Bigr),
\]
...
\Cref{thm:A} is derived directly from \cite[Thm.~2]{CS25}, including its integer-radius condition $f<n-k-1$, while \cref{thm:B} remains a separate slacked fallback imported through the BCHKS/ABF route. Both underlying sources are recent, and readers relying on the present results should consult them directly. The main composition is deliberately shallow --- only the displayed CS25 consequence is used --- and \cref{prop:slacked} provides a second, independent route; still, readers should treat \cref{cor:grand} as conditional on the correctness of the direct CS25 conversion recorded in \cref{thm:A}.

\paragraph{Open problems.}

\begin{problem}\label{prob:explicit}
Exhibit explicit pairs $(f,g)$ over $\F_{p^6}$ (KoalaBear sextic, parameters of \cref{cor:deployed}) with MCA-bad-slope density $>2^{-22}$ at gap $2^{-7}$. On the CS-admissible subinterval, ask for the corresponding CA-bad-slope density as well. By \cref{cor:Fvalued} such pairs exist and are not $\B$-rational; the natural candidates are residue-line normal forms of \cite{Cho26b} with denominator $E\in\F[X]$ not defined over $\B$.
\end{problem}
...
\paragraph{Acknowledgements.} This note composes results of Crites--Stewart \cite{CS25} and of \cite{BCHKS25} with the locator framework of \cite{Cho26a,Cho26b}, following the synthesis of Arnon--Boneh--Fenzi \cite{ABF26}; it sharpens the conditional universal-cap theorem and refines the subfield-confinement theorem of \cite{Cho26b}.
