...
\newcommand{\ImgFib}{\mathsf{ImgFib}}
...
Define also the codeword-image fiber
\[
        \ImgFib_U(a):=\{\widehat U\bmod L_S:\ S\in\Fib_U(a)\}
        \subseteq \F[X]_{<k}.
\]
\end{definition}
...
More precisely,
\[
        |\Lambda(C,1-a/n,U)|=|\ImgFib_U(a)|.
\]
Consequently, a uniform bound $|\ImgFib_U(a)|\le n^{B_L}$ over all $U$ implies $|\Lambda(C,1-a/n)|\le n^{B_L}$.
\end{lemma}

\begin{proof}
Every codeword $p\in\Lambda(C,1-a/n,U)$ agrees with $U$ on at least $a$ points;
choose an $a$-subset $S_p$ of its agreement set.  Then $S_p\in\Fib_U(a)$ and
$\widehat U\bmod L_{S_p}=p$, so $p\in\ImgFib_U(a)$.  Conversely, every residue
in $\ImgFib_U(a)$ has degree $<k$ and agrees with $U$ on its defining support.
Thus $\ImgFib_U(a)$ is exactly the list.  The inequality
$|\ImgFib_U(a)|\le|\Fib_U(a)|$ gives the displayed support-fiber upper bound.
\end{proof}
...
\]
and all active quotient-core profiles satisfy \eqref{eq:qprofile-list-budget}.  Equivalently, after quotient-periodic classes are accounted for, distinct locator-image fibers behave like random codimension-$\sigma_n$ fibers on $(k_n+\sigma_n)$-subsets.
\end{assumption}
...
A third floor, proved in the companion cap paper \cite{UniversalCap} conditional on the imported Crites--Stewart conversion, binds the line field itself.  Whenever some divisor $N\mid n$ has $n/N\mid k$ and $\binom{N}{\rho N+2}\ge\qgen\bigl(\qline/k+1\bigr)$, every radius $\delta\in[1-\rho-2/N,\,1-\rho)$ carries
\begin{equation}
\label{eq:cap-floor}
