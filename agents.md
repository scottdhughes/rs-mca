...
codeword span has rank at least six.  The predecessor inequality
`sum_i binom(w+s_i+r+e,r)<=binom(R+g-e,r)` is strengthened by marked
first-pivot and split-block counts: rank five is excluded, while rank six is
trapped in `781458<=g<=1033227` with aggregate excess at most
`96161189784`.  Ranks seven and above remain unpaid, and rank twelve receives
no uniform first-pivot cut over the full shallow range.  A realized tiny
family shows that marked-basis counting is sharp up to the excluded zero
anchor, while a nonrealized deployed scalar profile passes every present
aggregate gate.  The next varying-`G` theorem must therefore use positive-`w`
fixed-syndrome secant or split-rational incidence.  After
