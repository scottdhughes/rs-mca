...
census, not proof that the surviving pairs use distinct `G` values.  After
...
family may concentrate on one `G`; fixed-`G` already embeds an arbitrary
nowhere-zero ordinary RS received word and its Johnson denominator is
nonpositive throughout `72859<=m<=908270`.  Therefore the next theorem must
use split numerators, denominator coprimality, and global aggregation, or an
exhaustive source-bound quotient/Chebyshev/CA owner bridge.  Do not return to
another fixed-locator rank calculation or assume cross-`G` variation.
